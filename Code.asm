.data
#khoi tao cac thong tin can co ve bo nho
	p_day: .space 3
	p_month: .space 3
	p_year: .space 5

	time: .asciiz "26/04/2019"   # yeu cau cua 6
	time_1: .asciiz "--/--/----"
	...
	stuff: .asciiz	"------()CREATE BY 1712236 - 1712823 -1712224 - 17CNTN()------\n"
	option: .asciiz	"Lua chon: "
	type: .asciiz "(A,B,C): "
	result: .asciiz "\nKet qua: "
	m_continue: .asciiz "\nChon (1) - tiep tuc, (0) - thoat:  "
	leap: .asciiz " Nam nhuan."
	notleap: .asciiz " Khong phai nam nhuan."
...
prompt_again:
	...
	move	$a0, $v0
	move	$t0, $v0
	jal	checkdate	# kiem tra thoi gian nhap vao
...
menu:# tao menu chinh theo form text ? tren
	addi	$sp, $sp, -12
	sw	$ra, 8($sp)
	sw	$t0, 4($sp)
...
o5:
	...
	sub	$a0, $a0, $a1
	slt	$t0, $a0, $0	# if(checkDayTime(time) - checkDayTime(time_1) < 0)
	beq	$t0, $0, print
	li	$t0, -1
	mult	$a0, $t0
	mflo	$a0
...
# Cac dieu kien check nam nhuan la chia het 400 || khong chia het 100 vs chia het 4
	li	$t0, 400
	div	$a0, $t0
	mfhi	$t0
	...
	div	$a0, $t0
	mfhi	$t0
	bne 	$t0, $0, false	# so du != 0 <=> khong chia het 4 -> false		
